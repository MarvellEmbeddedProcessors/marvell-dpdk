...
emdev_lf_apinotif_process_desc(struct psw_lf *lf)
{
	...
	nb_desc = DESC_DIFF(pi, ci, aq_qp->q_sz);
	while (nb_desc > 0) {
#ifdef CNXK_EMDEV_DEBUG
		plt_dump("anq ci=0x%x pi=0x%x", ci, pi);
...
static void
emdev_lf_ras_irq(void *param)
{
	struct psw_lf *lf = (struct psw_lf *)param;
	uint64_t intr;

	intr = plt_read64(lf->rbase + PSW_LF_RAS_INT);
	if (intr == 0)
		return;

	plt_err("Err_irq=0x%" PRIx64 " lf=%d", intr, lf->lf_id);
	/* Clear interrupt */
	plt_write64(intr, lf->rbase + PSW_LF_RAS_INT);
}

static int
emdev_lf_register_ras_irq(struct psw_lf *lf)
{
	struct emdev *emdev = lf->emdev;
	struct plt_pci_device *pci_dev = emdev->pci_dev;
	struct plt_intr_handle *handle;
	...
	vec = lf->msixoff + PSW_LF_RAS_INT_VEC;
	handle = pci_dev->intr_handle;

	plt_write64(1, lf->rbase + PSW_LF_RAS_INT_ENA_W1C);
	/* Register handler for RAS interrupt */
	rc = dev_irq_register(handle, emdev_lf_ras_irq, lf, vec);
	plt_write64(1, lf->rbase + PSW_LF_RAS_INT_ENA_W1S);

	return rc;
}

static void
emdev_lf_unregister_ras_irq(struct psw_lf *lf)
{
	struct plt_pci_device *pci_dev = lf->emdev->pci_dev;
	struct plt_intr_handle *handle;
	int vec;
	...
	vec = lf->msixoff + PSW_LF_RAS_INT_VEC;
	plt_write64(1, lf->rbase + PSW_LF_RAS_INT_ENA_W1C);
	dev_irq_unregister(handle, emdev_lf_ras_irq, lf, vec);
}
...
emdev_lf_irqs_unregister(struct emdev *emdev)
{
	...
	for (i = 0; i < emdev->nb_psw_lfs; i++) {
		...
		emdev_lf_unregister_ras_irq(psw_lf);
	}
}
...
emdev_lf_irqs_register(struct emdev *emdev)
{
	...
	for (i = 0; i < emdev->nb_psw_lfs; i++) {
		...
		rc = emdev_lf_register_ras_irq(psw_lf);
		if (rc) {
			plt_err("Error registering PSWLF RAS irq for lf=%d, rc=%d", i, rc);
			emdev_lf_unregister_apinotif_irq(psw_lf);
			goto exit;
		}
	}
...
exit:
	i--;
	for (; i >= 0; i--) {
		emdev_lf_unregister_apinotif_irq(&emdev->psw_lfs[i]);
		emdev_lf_unregister_ras_irq(&emdev->psw_lfs[i]);
	}
	return rc;
}
